...
| (7) 과제 내용 요약 |  **길거리 판매왕**은 플레이어가 AI NPC와의 설득 대화를 통해 물건을 판매하며, 현실감 있는 협상을 경험하는  롤플레잉 토킹 게임입니다. 길거리 판매왕은 매번 랜덤하게 매칭되는 NPC와의 상호작용을 통해, 플레이어가 각기 다른 외모와 성격, 대화 스타일을 가진 NPC를 만나며 설득의 재미를 느낄 수 있는 환경을 제공합니다.<br><br> 주요 기능으로는 **ChatGPT-4 API**와 **Unity3D**을 활용한 실시간 대화 구현이 있습니다. STT 기술은 플레이어의 음성을 텍스트로 변환해 NPC와 자연스럽게 대화가 이어지도록 하며, 설득력 기반의 점수 시스템은 플레이어의 대화 방식에 따라 NPC의 반응과 협상 결과를 실시간으로 변화시킵니다. 예를 들어, 설득력이 높은 발언은 NPC의 가격 협상 성공을 돕고, 설득에 실패할 경우 NPC와의 과거 협상이 어려워집니다.<br><br> 이러한 시스템을 통해 길거리 판매왕은 매번 똑같은 대화 스크립트를 제공하는 전통적인 게임의 문제점을 해결할 수 있습니다. 대화 내용에 따라 NPC의 반응이 달라져 몰입감을 높이고, 게임 경험을 플레이어에 맞게 개별화함으로써 한정된 선택지로 인해 경험이 제한되는 기존의 게임 문제점을 보완하였습니다. |
